...
One of the P017 functions is used to load all plays by Holberg, located in the `data`-folder, into a table called allplays. Then, characters, that are referred to with their alias, are renamed to their main name.
...
To save a plot as a pdf file (which is preferable, since the quality of the plot will remain high when zooming in), run the function below which saves the last drawn plot in the specified location. If png format is preferred, simply change the file name extension from pdf to png. 
```{r}
ggsave(here("graphs/word-percentage-plots", "vis-word-percent-plot.pdf"),
       width = 297, 
       height = 210, 
       units = "mm")
```
